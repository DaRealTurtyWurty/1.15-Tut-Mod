...
public class ModBeehiveBlock extends HorizontalBlock {
	...
	public ModBeehiveBlock(Block.Properties properties) {
		super(properties);
		this.setDefaultState(this.stateContainer.getBaseState().with(HONEY_LEVEL, Integer.valueOf(0))
	...
	public void harvestBlock(World worldIn, PlayerEntity player, BlockPos pos, BlockState state,
		...
		if (!worldIn.isRemote && te instanceof ModBeehiveTileEntity) {
			ModBeehiveTileEntity beehivetileentity = (ModBeehiveTileEntity) te;
			if (EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, stack) == 0) {
				beehivetileentity.angerBees(player, state, ModBeehiveTileEntity.State.EMERGENCY);
	...
	private void angerNearbyBees(World p_226881_1_, BlockPos p_226881_2_) {
		List<BeeEntity> list = p_226881_1_.getEntitiesWithinAABB(BeeEntity.class,
				(new AxisAlignedBB(p_226881_2_)).grow(8.0D, 6.0D, 8.0D));
		if (!list.isEmpty()) {
			List<PlayerEntity> list1 = p_226881_1_.getEntitiesWithinAABB(PlayerEntity.class,
					(new AxisAlignedBB(p_226881_2_)).grow(8.0D, 6.0D, 8.0D));
			int i = list1.size();
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player,
		...
		if (flag) {
			if (!CampfireBlock.isLitCampfireInRange(worldIn, pos, 5)) {
				...
				this.takeHoney(worldIn, state, pos, player, ModBeehiveTileEntity.State.EMERGENCY);
			} else {
	...
	private boolean hasBees(World p_226882_1_, BlockPos p_226882_2_) {
		...
		if (tileentity instanceof ModBeehiveTileEntity) {
			ModBeehiveTileEntity beehivetileentity = (ModBeehiveTileEntity) tileentity;
	...
	public void takeHoney(World p_226877_1_, BlockState p_226877_2_, BlockPos p_226877_3_,
			@Nullable PlayerEntity p_226877_4_, ModBeehiveTileEntity.State p_226877_5_) {
	...
	private void func_226879_a_(World p_226879_1_, BlockPos p_226879_2_, BlockState p_226879_3_) {
		if (p_226879_3_.getFluidState().isEmpty() && !(p_226879_1_.rand.nextFloat() < 0.3F)) {
			...
			if (d0 >= 1.0D && !p_226879_3_.isIn(BlockTags.IMPERMEABLE)) {
				...
				if (d1 > 0.0D) {
					this.addHoneyParticle(p_226879_1_, p_226879_2_, voxelshape,
							(double) p_226879_2_.getY() + d1 - 0.05D);
				} else {
					...
					if ((d2 < 1.0D || !blockstate.isCollisionShapeOpaque(p_226879_1_, blockpos))
							&& blockstate.getFluidState().isEmpty()) {
						this.addHoneyParticle(p_226879_1_, p_226879_2_, voxelshape,
								(double) p_226879_2_.getY() - 0.05D);
					}
				}
			}
	...
	private void addHoneyParticle(World p_226880_1_, BlockPos p_226880_2_, VoxelShape p_226880_3_, double p_226880_4_) {
		this.addHoneyParticle(p_226880_1_, (double) p_226880_2_.getX() + p_226880_3_.getStart(Direction.Axis.X),
				(double) p_226880_2_.getX() + p_226880_3_.getEnd(Direction.Axis.X),
				(double) p_226880_2_.getZ() + p_226880_3_.getStart(Direction.Axis.Z),
				(double) p_226880_2_.getZ() + p_226880_3_.getEnd(Direction.Axis.Z), p_226880_4_);
	}

	@OnlyIn(Dist.CLIENT)
	private void addHoneyParticle(World p_226875_1_, double p_226875_2_, double p_226875_4_, double p_226875_6_,
			double p_226875_8_, double p_226875_10_) {
		p_226875_1_.addParticle(ParticleTypes.DRIPPING_HONEY,
				MathHelper.lerp(p_226875_1_.rand.nextDouble(), p_226875_2_, p_226875_4_), p_226875_10_,
				MathHelper.lerp(p_226875_1_.rand.nextDouble(), p_226875_6_, p_226875_8_), 0.0D, 0.0D, 0.0D);
	}
	...
	protected void fillStateContainer(StateContainer.Builder<Block, BlockState> builder) {
		builder.add(HONEY_LEVEL, HORIZONTAL_FACING);
	}

	@Nullable
	public TileEntity createNewTileEntity(IBlockReader worldIn) {
		return ModTileEntityTypes.MOD_BEEHIVE.get().create();
	}
	...
	public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
		...
		if (entity instanceof TNTEntity || entity instanceof CreeperEntity || entity instanceof WitherSkullEntity
			...
			if (tileentity instanceof ModBeehiveTileEntity) {
				ModBeehiveTileEntity beehivetileentity = (ModBeehiveTileEntity) tileentity;
				beehivetileentity.angerBees((PlayerEntity) null, state, ModBeehiveTileEntity.State.EMERGENCY);
			}
		}
	...
	public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn,
		...
		if (worldIn.getBlockState(facingPos).getBlock() instanceof FireBlock) {
			...
			if (tileentity instanceof ModBeehiveTileEntity) {
				ModBeehiveTileEntity beehivetileentity = (ModBeehiveTileEntity) tileentity;
				beehivetileentity.angerBees((PlayerEntity) null, stateIn, ModBeehiveTileEntity.State.EMERGENCY);
			}
		}
